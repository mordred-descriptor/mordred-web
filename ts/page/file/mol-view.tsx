...
function MolDisplay({mol, is3D}: {mol: MolProps["mol"], is3D: boolean}): JSX.Element {
    ...
    if (is3D) {
        body = <MV backgroundColor="white" className="mol-display" src={mol || undefined} loaderParams={{ext: "sdf"}}/>;
    } else if (mol) {
    ...
    return <div className="mol-display-wrapper">{overlay}{body}</div>;
}
    ...
    return (
        <div className="mol-view-container">
            <div className="mol-name">
                <h4 className="text-center">{props.name} {props.forcefield ? <span className="forcefield label">{props.forcefield}</span> : null}</h4>
            </div>
